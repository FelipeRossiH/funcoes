...
print("\nQuantidade de cada item na coluna 'Tipo':")
print(quantidade_tipo)
print("\nQuantidade de cada item na coluna 'Situação':")
print(quantidade_situacao)


data = {
    'Tipo': ['Tipo1', 'Tipo2', 'Tipo3'],  # Substitua pelos seus dados
    'Quantidade': [10, 20, 30]  # Substitua pelos seus dados
}
df_tipo = pd.DataFrame(data)

# Criar gráfico de barras para a coluna 'Tipo'
plt.figure(figsize=(10, 6))
plt.bar(df_tipo['Tipo'], df_tipo['Quantidade'], color='skyblue')
plt.title('Quantidade de cada item na coluna "Tipo"')
plt.xlabel('Tipo')
...
data = {
    'Situação': ['Situação1', 'Situação2', 'Situação3'],  # Substitua pelos seus dados
    'Quantidade': [15, 25, 35]  # Substitua pelos seus dados
}
df_situacao = pd.DataFrame(data)

# Criar gráfico de barras para a coluna 'Situação'
plt.figure(figsize=(10, 6))
plt.bar(df_situacao['Situação'], df_situacao['Quantidade'], color='lightgreen')
plt.title('Quantidade de cada item na coluna "Situação"')
plt.xlabel('Situação')
plt.ylabel('Quantidade')
plt.xticks(rotation=45)
plt.tight_layout()
